...
export async function generateMetadata({
  ...
  const description =
    post.description || "인터랙티브 컴포넌트와 함께하는 기술 블로그";
...
export default async function Post({
  ...
  const isPasswordEnabled = !!post.password;
  const isPasswordAuthed = await isPostPasswordAuthenticated(slug);
  const canAccessHidden =
    isDev || isAdmin || (isPasswordEnabled && isPasswordAuthed);
    ...
    <>
      ...
      >
        ...
        <div
          ...
          className={cn("flex justify-center gap-0 relative")}
        >
          <article
            ...
            >
              ...
              {post.hidden && (isDev || isAdmin || isPasswordAuthed) && (
                ...
                >
                  ...
                  >
                    🔒 이 포스트는 Hidden 상태입니다. 프로덕션에서는 보이지
                    않습니다.
                  </div>
                </div>
              )}
              ...
              <BibTeXCopyButton bibtex={generateBibTeX(post)} post={post}>
                ...
                >
                  ...
                  {post.affiliate && <span>· {post.affiliate}</span>}
                </div>
              </BibTeXCopyButton>
