---
title: 'A Deep Dive into setTimeout and setInterval'
...
위 도구를 사용하여 현재 브라우저에서 타이머의 실제 정확도를 직접 측정해볼 수 있습니다. 요청한 지연 시간과 실제 측정된 시간의 차이를 확인하고, 특히 4ms 미만의 매우 짧은 지연 시간에서 브라우저의 최소 지연 시간 클램핑이 어떻게 작동하는지 관찰해보세요.
...
위 시뮬레이터는 타이머를 정리하지 않았을 때의 메모리 누수를 시각적으로 보여줍니다. "자동 생성" 버튼을 눌러 계속해서 타이머를 생성하면서 정리하지 않으면 어떤 일이 벌어지는지 확인해보세요. 실제 애플리케이션에서는 이러한 누수가 성능 저하와 메모리 부족으로 이어질 수 있습니다.
...
위 플레이그라운드에서는 `setTimeout`, `setInterval`, `requestAnimationFrame`의 실제 실행 간격을 실시간으로 비교할 수 있습니다. 각 메서드를 활성화하고 목표 간격을 조절하면서 실제로 어떻게 동작하는지 직접 확인해보세요. 특히 `requestAnimationFrame`은 브라우저의 리페인트 주기(보통 60fps, 16.67ms)에 맞춰 실행되므로 애니메이션에 가장 적합합니다.
